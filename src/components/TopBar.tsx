...
export function TopBar({ currentView, onViewChange, projectSource, syncStatus, onCloseProject }: TopBarProps) {
  ...
  const projectFolderPath = useStore((state) => state.projectFolderPath);
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-1 w-48 bg-slate-800 border border-slate-700 rounded shadow-lg z-50">
              <div className="py-1">
                ...
                <a
                  href="https://github.com/shuffleo/storyboard-tool"
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-2 w-full text-left px-4 py-2 text-sm text-slate-200 hover:bg-slate-700"
                >
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto modal-content">
                ...
                <div className="p-4 space-y-4">
                  ...
                  {projectFolderPath && (
                    <div>
                      <label className="block text-xs font-medium text-slate-300 mb-1 uppercase">Folder</label>
                      <div className="flex items-center gap-2 px-3 py-2 bg-slate-900 border border-slate-600 rounded-md">
                        ...
                        <span className="text-sm text-slate-300 font-mono truncate" title={projectFolderPath}>
                          {projectFolderPath}
                        </span>
                      </div>
                    </div>
